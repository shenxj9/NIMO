...
def _add_logging_opts(parser, is_train=True):
    ...
    if is_train:
        ...
        group.add('--tensorboard', '-tensorboard', action="store_false",
...
def _add_dynamic_corpus_opts(parser, build_vocab_only=False):
    ...
    group.add("-transforms", "--transforms", default=['onmt_tokenize'], nargs="+",
    ...
    group.add("-save_data", "--save_data", required=False,default=' ',
              help="Output base path for objects that will "
                   "be saved (vocab, transforms, embeddings, ...).")
    group.add("-overwrite", "--overwrite", action="store_false",
...
def _add_dynamic_fields_opts(parser, build_vocab_only=False):
    ...
    group.add("-tgt_vocab", "--tgt_vocab",
              ...
              + " tgt vocabulary file. "
              "Format: one <word> or <word>\t<count> per line.")
    group.add("-share_vocab", "--share_vocab", action="store_false",
...
def model_opts(parser):
    ...
    group.add('--word_vec_size', '-word_vec_size', type=int, default=512,
    ...
    group.add('--share_embeddings', '-share_embeddings', action='store_true',
              help="Share the word embeddings between encoder "
                   ...
                   "option.")
    group.add('--position_encoding', '-position_encoding', action='store_false',
    ...
    group.add(
        "-model_task",
        "--model_task",
        default=ModelTask.LANGUAGE_MODEL,
    ...
    group.add('--encoder_type', '-encoder_type', type=str, default='transformer_lm',
              choices=['rnn', 'brnn', 'ggnn', 'mean', 'transformer', 'cnn',
                       'transformer_lm'],
              help="Type of encoder layer to use. Non-RNN layers "
                   "are experimental. Options are "
                   "[rnn|brnn|ggnn|mean|transformer|cnn|transformer_lm].")
    group.add('--decoder_type', '-decoder_type', type=str, default='transformer_lm',
...
def _add_train_general_opts(parser):
    ...
    group.add('--param_init_glorot', '-param_init_glorot', action='store_false',
    ...
    group.add('--batch_size', '-batch_size', type=int, default=2048,
    ...
    group.add('--valid_batch_size', '-valid_batch_size', type=int, default=512,
    ...
    group.add('--adam_beta2', '-adam_beta2', type=float, default=0.998,
    ...
    group.add('--learning_rate', '-learning_rate', type=float, default=2.0,
...
def _add_decoding_opts(parser):
    ...
    beam_size = group.add('--beam_size', '-beam_size', type=int, default=3,
                          help='Beam size')
    ...
    group.add('--length_penalty', '-length_penalty', default='wu',
              choices=['none', 'wu', 'avg'],
    ...
    group.add('--min_length', '-min_length', type=int, default=4,
              help='Minimum prediction length')
    group.add('--max_length', '-max_length', type=int, default=40,
    ...
    group.add('--ban_fragments', '-ban_fragments',
              type=bool, default=True,
              help="Improve the success rate of fragment reconstruction")
    group.add('--phrase_table', '-phrase_table', type=str, default="",
              help="If phrase_table is provided (with replace_unk), it will "
                   "look up the ident   ified source token and give the "
...
def translate_opts(parser):
    ...
    group.add('--scaffold', '-scaffold', required=False)
