...
from onmt.utils.recombination import recombination
...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    for model_path in opt.models:
       fields, model, model_opt =  load_test_model(opt, model_path)
...
class Inference(object):
    ...
    def from_opt(
        ...
        return cls(
            ...
            ban_fragments=opt.ban_fragments,
    ...
    def dummy_ship(self):
        ...
        for (key, value) in vocab.stoi.items():
            if key !=None:
                if re.match('dummy', key):
                    markers_dict[key] = value
        alternate = markers_dict['dummy_1']
        for i in markers_dict.keys():
            dummy_dict[i] = int(re.search('\d+', i).group())
        for (key, value) in dummy_dict.items():
            dummy_ship[markers_dict[key]] = value
        return dummy_ship,alternate
    ...
    def translate(
        ...
        for batch in data_iter:
            ...
            for trans in translations:
                ...
                recombination(n_best_preds, self.out_file, self.scaffold)
        ...
        with torch.no_grad():
            if self.sample_from_topk != 0 or self.sample_from_topp != 0:
                ...
                decode_strategy = BeamSearch(
                    ...
                    ratio=self.ratio,
                    ban_unk_token=self.ban_unk_token,
                    ban_fragments=self.ban_fragments,
                    field = self.fields
...
class GeneratorLM(Inference):
    ...
    def _translate_batch_with_strategy(
        ...
        dummy_ships,alternate = self.dummy_ship()
        ...
        for step in range(decode_strategy.max_length):
            ...
            decode_strategy.advance(log_probs, attn, dummy_ships,alternate)
            any_finished = decode_strategy.is_finished.any()
            if any_finished:
                decode_strategy.update_finished()
                if decode_strategy.done:
