...
class DecodeStrategy(object):
    ...
    def __init__(self, pad, bos, eos, unk, batch_size, parallel_paths,
                 ...
                 ban_unk_token,ban_fragments,field):
        ...
        self.ban_fragments = ban_fragments
        self.field = field
    ...
    def ensure_fragment_recombination(self,log_probs,alive_seq,dummy_ships,alternate):

        if self.ban_fragments and len(self) %2 == -1 and len(self) > 4:
            alive_seq = alive_seq.tolist()
            ban_fragments = []
            pick_eos = []
            keys = []
            for (key, value) in dummy_ships.items():
                keys.append(key)
            for seq in alive_seq:
                picked = [dummy_ships[val] for val in seq if val in keys]
                fragments = sum(picked) - 2 * len(picked)
                # FragMG-NPM/FragMG-NPS
                if fragments == -1:
                    ban_fragments.append(True)
                    pick_eos.append(True)
                else:
                    ban_fragments.append(False)
                    pick_eos.append(False)

                # ring-separation and ring-fusion functions
                # if fragments == 1:
                #     ban_fragments.append(True)
                #     pick_eos.append(True)
                # elif fragments == -1:
            ...
            log_probs[pick_eos, self.eos] = -1e20
            log_probs[ban_fragments, alternate] = torch.max(torch.max(log_probs,dim=1).values)*1.1
